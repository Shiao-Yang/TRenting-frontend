            ...
            <el-tabs v-model="activeName" stretch="true" >
              <el-tab-pane label="全部订单" name="first">
                <el-table
                  ...
                  <el-table-column align="center" prop="oid" label="订单编号" >
                  </el-table-column>
                  <el-table-column align="center" prop="status" label="订单状态" width="180">
                    <template v-slot="scope">
                      ...
                      <el-divider direction="vertical" v-if="scope.row.status===1 && scope.row.paid === 1"></el-divider>
                      <el-tag type="warning" v-if="scope.row.status===1 && scope.row.paid === 1 && scope.row.valid === 0" >
                        待生效
                      </el-tag>
                      <el-tag type="success" v-if="scope.row.status===1 && scope.row.paid === 1 && scope.row.valid === 1" >
                        已生效
                      </el-tag>
                      <el-tag type="info" v-if="scope.row.status===1 && scope.row.paid === 1 && scope.row.valid === 2" >
                        已过期
                      </el-tag>
                  ...
                  <el-table-column align="center" prop="paid" label="支付状态" >
                    <template v-slot="scope">
                      ...
                      <el-tag type="success" v-else-if="scope.row.paid===1&&scope.row.status===1">
                        已付款
                      </el-tag>
                      <el-tag type="info" v-if="scope.row.status===2">
                        已取消
                    ...
                    <template v-slot="scope">
                      ...
                      <div v-if="scope.row.status===1&&scope.row.paid===1&&scope.row.valid === 2">
                        <el-popconfirm title="确定删除订单吗？" icon-color="red" @confirm="deleteOrder(scope.row.oid)">
                          <el-button type="danger" plain size="mini" slot="reference" >删除订单</el-button>
                        </el-popconfirm>
                      </div>
                    ...
                    <template v-slot="scope">
                      <el-popover placement="left" width="500" trigger="click">
                        ...
                        <el-card shadow="hover" :body-style= "{ padding:  '0px'}" width="100%">
                          <img class="show_img" src="../assets/cart.png" alt="加载失败">
                          <div style="padding: 14px;">
                            ...
                            <div v-if="scope.row.type===0" class="card-item">
                              租房类型:  &nbsp;&nbsp;短租
                            </div>
                            <div v-if="scope.row.type===1" class="card-item">
                              租房类型:  &nbsp;&nbsp;长租
                            </div>
                            <div v-if="scope.row.type===0" class="card-item">
                              短租价格:  &nbsp;&nbsp;{{ scope.row.h_short_price }} 元 / 天
                            </div>
                            <div v-if="scope.row.type===1" class="card-item">
                              长租价格:  &nbsp;&nbsp;{{ scope.row.h_long_price }} 元 / 月
                            </div>
                            <div v-if="scope.row.type===0" class="card-item">
                              租房时长:  &nbsp;&nbsp;{{scope.row.duration}} 天
                            </div>
                            <div v-if="scope.row.type===1" class="card-item">
                              租房时长:  &nbsp;&nbsp;{{scope.row.duration}} 个月
                            </div>
                            <div class="card-item">
                              租房开始日期:  &nbsp;&nbsp;{{scope.row.start_time}}
                            </div>
                            <div class="card-item">
                              租房结束日期:  &nbsp;&nbsp;{{scope.row.end_time}}
                            </div>
                          </div>
              ...
              <el-tab-pane label="当前有效订单" name="forth">
                <el-table
                    :data="order_valid_paging"
                    align="center"
                    border
                    type="index"
                ...
                <el-pagination
                    ...
                    :page-size="5"
                    :pager-count="7"
                    :current-page="current_page_valid"
                    layout="prev, pager, next"
                    @current-change = "order_valid_change_page"
                    :total="this.order_valid.length"
                    style="float:right">
                </el-pagination>
              </el-tab-pane>
...
export default {
  ...
  methods:{
    ...
    getOrders() {
      ...
      console.log(formData);
      let Url = "http://127.0.0.1:8000/order/?uid="+formData.uid;
      this.$axios({
        method: 'get',
        url: Url,
      })
      .then(res => {
        ...
        for(let i = 0; i < this.order_all.length; i++) {
          ...
          //console.log(this.order_all[i].start_time);
          //console.log(this.order_all[i].end_time);
          if (this.order_all[i].paid === 0 && (this.order_all[i].status === 1 || this.order_all[i].status === 0)) {
            this.order_unpaid.push(this.order_all[i]);
            console.log(1);
          }

          if (this.order_all[i].paid === 1 && this.order_all[i].status === 1) {
          ...
          t = this.order_all[i]['start_time'];
          if(this.order_all[i].type === 0) { //短租
            end_time = this.DateCal(t, this.order_all[i].duration);
          }
          if(this.order_all[i].type === 1) { //短租
            end_time = this.MonthCal(t, this.order_all[i].duration);
          }
          //console.log(this.order_all[i]['end_time']);

          let sign1 = this.DateCompare(this.Date2String(date), this.Date2String(this.order_all[i]['start_time']));
          let sign2 = this.DateCompare(this.Date2String(date), end_time);
          ...
          //console.log(this.order_all[i]);
          /*
          let sign1 = this.DateCompare(this.Date2String(date), this.order_all[i]['start_time']);
          let sign2 = this.DateCompare(this.Date2String(date), this.order_all[i]['end_time']);
          ...
          //console.log(this.order_all[i].valid,'c');
          //console.log(this.order_all[i].oid,this.order_all[i].valid,this.order_all[i].status,this.order_all[i].paid,'c');
          if (this.order_all[i].status === 1 && this.order_all[i].paid === 1 && this.order_all[i].valid === 1) {
            this.order_valid.push(this.order_all[i]);
            //console.log(3);
          }
        ...
        //console.log(a);
          this.order_valid_paging = [];
          this.current_page_valid = 1;
          //let a = JSON.parse(this.order_all);
          //console.log(a);
          //console.log(this.order_all[0].valid);
          //console.log('key' in this.order_all[0]);
        //console.log(JSON.stringify(this.order_all));
          //console.log(this.order_all[0].hasOwnProperty("start_time"));
          //console.log(this.order_all[0].end_time);
          /*
        for(let i = 0; i < this.order_all.length&&i<5; i++) {
          console.log(this.order_all[i]["end_time"]);
        }
           ...
           */
        //console.log(this.order_all[0].start_time);
          //this.fill_order_valid();
    ...
    order_valid_change_page(new_page) {
      this.order_valid_paging = [];
      this.current_page_valid = new_page;
      for(let i = 5*(new_page-1); i < this.order_valid.length && i < 5*new_page; i++) {
        this.order_valid_paging.push(this.order_valid[i]);
      }
    },
    getUser(uid) {
