      ...
      <div class="w">
        ...
        <div class="main">
          ...
          <el-dialog
            ...
            <div class="pay_img1">
              <img id="img1" src="../assets/submit.png" alt="图片加载中" width="50%">
            </div>
            <div slot="footer" class="dialog-footer">
              <el-button @click="dialogVisible = false">取 消</el-button>
              <el-button type="primary" @click="dialogVisible = false; payOrder(order_id_to_pay)">确 定</el-button>
            </div>
          </el-dialog>

          <div class="order_tabs">
            <el-tabs v-model="activeName" stretch="true" >
              <el-tab-pane label="全部报修" name="first">
                <el-table
                    :data="ticket_all_paging"
                    align="center"
                    border
                    v-loading="this.loading"
                    type="index"
                    width="100%">
                  <el-table-column align="center" prop="id" width="150" label="报修编号">
                  </el-table-column>
                  <el-table-column align="center" prop="hid" width="150" label="房源编号" >
                  </el-table-column>
                  <el-table-column align="center" prop="status" label="报修状态" >
                  ...
                  <el-table-column align="center" prop="info" label="文字描述" :show-overflow-tooltip="true" width="250">
                  </el-table-column>
                  <el-table-column align="center" label="操作" width="200">
                    <template v-slot="scope" >
                      <div v-if="scope.row.status !== 3">
                        <el-button type="info" size="small" plain>暂无操作</el-button>
                      </div>
                      <div v-else-if="scope.row.comment===null">
                        <el-button type="primary" size="small" plain>评价师傅</el-button>
                      </div>
                      <div v-else>
                        <el-button type="success" size="small" plain >评价完毕</el-button>
                      </div>
                    </template>
                  </el-table-column>
                      ...
                      <el-popover placement="left" width="500" trigger="click">
                        ...
                        <el-card shadow="hover" :body-style= "{ padding:  '0px'}" width="100%">
                          ...
                          <div style="padding: 14px;">
                            ...
                            <div class="card-item">
                              房源位置:  &nbsp;&nbsp;{{scope.row.h_location}}
                            </div>
                            <div v-if="scope.row.h_type === 1" class="card-item">
                              房源户型:  &nbsp;&nbsp;单人间
                            </div>
                            <div v-if="scope.row.h_type === 2" class="card-item">
                              房源户型:  &nbsp;&nbsp;双人间
                            </div>
                            <div v-if="scope.row.h_type === 3" class="card-item">
                              房源户型： &nbsp;&nbsp;四人间
                            </div>
                            <div class="card-item">
                              房源面积:  &nbsp;&nbsp;{{scope.row.h_area}} m²
                            </div >
                          </div>
                          -->
                          <el-card shadow="hover" :body-style= "{ padding:  '0px'}" width="100%">
                          ...
                          <el-card shadow="hover" :body-style= "{ padding:  '0px'}" width="100%">
                            <div style="padding: 14px;">
                              ...
                              <div>
                                报修图片: &nbsp;&nbsp;<el-image style="position: relative; top: 7px;" :src="scope.row.pictures" alt="加载失败"></el-image>
                              </div>
                            </div>
                          </el-card>
                          ...
                          <el-card shadow="hover" :body-style= "{ padding:  '0px'}" width="100%">
                            <div style="padding: 14px;">
                              <div v-if="scope.row.wid === null">
                                维修师傅编号: &nbsp;&nbsp;暂无
                              </div>
                              <div v-if="scope.row.wid !== null">
                                维修师傅编号: &nbsp;&nbsp;{{scope.row.wid}}
                              </div>
                              <div v-if="scope.row.name === null || scope.row.name === ''">
                                维修师傅姓名: &nbsp;&nbsp;暂无
                              </div>
                              <div v-if="scope.row.name !== null && scope.row.name !== ''">
                                维修师傅姓名: &nbsp;&nbsp;{{scope.row.w_name}}
                              </div>
                              <div v-if="scope.row.date === null || scope.row.date === ''">
                                维修日期: &nbsp;&nbsp;暂无
                              </div>
                              <div v-if="scope.row.date !== null && scope.row.date !== ''">
                                维修日期: &nbsp;&nbsp;{{ scope.row.date }}
                              </div>
                              <div v-if="scope.row.materials_text === null || scope.row.materials_text === ''">
                                维修描述: &nbsp;&nbsp;暂无
                              </div>
                              <div v-if="scope.row.materials_text !== null && scope.row.materials_text !== ''">
                                维修描述: &nbsp;&nbsp;{{ scope.row.date }}
                              ...
                              <div>
                                报修图片: &nbsp;&nbsp;<el-image style="position: relative; top: 7px;" :src="scope.row.materials_pic" alt="加载失败"></el-image>
                              </div>
                            </div>
                          </el-card>
                        </el-card>
                ...
                <el-pagination
                    ...
                    @current-change = "ticket_all_change_page"
                    :total="this.ticket_all.length"
                    style="float:right">
                </el-pagination>
              </el-tab-pane>
...
export default {
  ...
  data () {
    return {
      ...
      ticket_handled: [], //已处理报修
      ticket_unhandled: [],   //未处理报修
      ticket_handling: [], //正在处理的报修
      ticket_valid: [],
      //订单数量
      ticket_all_count: 0,
      ticket_handled_count: 0,
      ticket_unhandled_count: 0,
      ticket_handling_count: 0,
      ticket_valid_count: 0,
      //当前页数
      current_page_all: 1,
      current_page_handled: 1,
      current_page_unhandled: 1,
      current_page_handling: 1,
      current_page_valid: 1,
      page_size: 4,
      //当前 各个订单页面中所展示的订单
      ticket_all_paging: [],
      ticket_handled_paging: [],
      ticket_unhandled_paging: [],
      ticket_handling_paging: [],
      ticket_valid_paging: [],
    }
  },
  ...
  methods:{
    ...
    test() {
      //console.log('hello!');
      console.log(this.ticket_all);
      //console.log(this.loading);
      //console.log(val);
    },
    storeTicket(ticket_id) {
      this.ticket_id_to_pay = ticket_id;
    },
    payOrder(ticket_id) {
      let formData = {
        oid: ticket_id,
    ...
    getTickets() {
      ...
      console.log(formData);
      let Url = "http://127.0.0.1:8000/tickets/get_tickets/";
      ...
      this.$axios({
        ...
        data: qs.stringify(formData),
          ...
          .then(res => {
            console.log(res.data);
            this.$store.state.ticket_all = res.data;
            this.$store.state.ticket_all_count = res.data.length;

            this.ticket_all=res.data;
            this.ticket_all_count = res.data.length;

            for(let i = 0; i < this.ticket_all.length; i++) {
              let tmp = this.ticket_all[i];
              let hid = tmp.hid;
              let url = "http://127.0.0.1:8000/browse_house/get_house/?hid="+hid;
              axios({
                method: 'get',
                ...
                this.ticket_all[i]['h_location'] = house.location;
                this.ticket_all[i]['h_short_price'] = house.short_price;
                this.ticket_all[i]['h_long_price'] = house.long_price;
                this.ticket_all[i]['h_type'] = house.type;
                this.ticket_all[i]['h_area'] = house.area;
              ...
              console.log(this.ticket_all[i]);
              if (this.ticket_all[i].status === 1) {
                this.ticket_unhandled.push(this.ticket_all[i]);
              }
              else if (this.ticket_all[i].status === 2) {
                this.ticket_handling.push(this.ticket_all[i]);
              }
              else if (this.ticket_all[i].status === 3) {
                this.ticket_handled.push(this.ticket_all[i]);
              }
            }
            this.ticket_all_paging = [];
            this.current_page_all = 1;
            if(this.ticket_all) {
              for(let i = 0; i < this.ticket_all.length&&i<5; i++) {
                this.ticket_all_paging.push(this.ticket_all[i]);
              }
            }
            this.ticket_unhandled_paging = [];
            this.current_page_unpaid = 1;
            if(this.ticket_unhandled) {
              for(let i = 0; i < this.ticket_unhandled.length&&i<5; i++) {
                this.ticket_unhandled_paging.push(this.ticket_unhandled[i]);
              }
            }
            this.ticket_handled_paging = [];
            this.current_page_handled = 1;
            if(this.ticket_handled) {
              for(let i = 0; i < this.ticket_handled.length&&i<5; i++) {
                this.ticket_handled_paging.push(this.ticket_handled[i]);
              }
            }
            this.ticket_handling_paging = [];
            this.current_page_handling = 1;
            if(this.ticket_handling) {
              for(let i = 0; i < this.ticket_handling.length&&i<5; i++) {
                this.ticket_handling_paging.push(this.ticket_handling[i]);
              }
            }
    ...
    ticket_all_change_page(new_page) {
      console.log(new_page);
      this.ticket_all_paging = [];
      this.current_page_all = new_page;
      for(let i = 5*(new_page-1); i < this.ticket_all.length && i < 5*new_page; i++) {
        this.ticket_all_paging.push(this.ticket_all[i]);
      }
      console.log(this.ticket_all_paging);
    },
    ticket_unhandled_change_page(new_page) {
      this.ticket_unhandled_paging = [];
      this.current_page_unhandled = new_page;
      for(let i = 5*(new_page-1); i < this.ticket_unhandled.length && i < 5*new_page; i++) {
        this.ticket_unhandled_paging.push(this.ticket_unhandled[i]);
      }
    },
    ticket_handled_change_page(new_page) {
      this.ticket_handled_paging = [];
      this.current_page_handled = new_page;
      for(let i = 5*(new_page-1); i < this.ticket_handled.length && i < 5*new_page; i++) {
        this.ticket_handled_paging.push(this.ticket_handled[i]);
      }
    },
    ticket_handling_change_page(new_page) {
      this.ticket_handling_paging = [];
      this.current_page_handling = new_page;
      for(let i = 5*(new_page-1); i < this.ticket_handling.length && i < 5*new_page; i++) {
        this.ticket_handling_paging.push(this.ticket_handling[i]);
      }
    },
    logOut() {
